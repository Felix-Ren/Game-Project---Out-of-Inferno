...
B3::B3(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::B3)
{
    ui->setupUi(this);

    const QString levelName = "B3";
    ...
    ui->RIP5->setPixmap(*RIP_pic);
    ui->RIP5->setScaledContents(true);

    ui->RIP6->setPixmap(*RIP_pic);
    ui->RIP6->setScaledContents(true);
    ...
    ui->coffin3->setPixmap(*coffin_pic);
    ui->coffin3->setScaledContents(true);
    ...
    ui->zombie_label_4->setPixmap(*zombie1_pic);
    ui->zombie_label_4->setScaledContents(true);
    ...
    // Call this level B1. Display it on the screen
    info.setLevel("B1");
...
*/
void B3::keyPressEvent(QKeyEvent* event){
    ...
    if((labelIntersected(ui->player_label, ui->RIP1) == false) &&
       ...
       (labelIntersected(ui->player_label, ui->RIP5) == false) &&
       (labelIntersected(ui->player_label, ui->RIP6) == false) &&
       (labelIntersected(ui->player_label, ui->coffin1) == false) &&
       (labelIntersected(ui->player_label, ui->coffin2) == false) &&
       (labelIntersected(ui->player_label, ui->coffin3) == false) &&
       (margin->geometry().contains(ui->player_label->geometry()) == true))
    {
        ...
        labelRandMove(ui->zombie_label_4);
        ...
        if((labelIntersected(ui->player_label, ui->zombie_label_1) == true) ||
           ...
           (labelIntersected(ui->player_label, ui->zombie_label_4) == true) ||
        ...
        player to success dialog.
        if(labelIntersected(ui->player_label, ui->ladder_label) == true && (info.numberOfLife>0)){
                SuccessDialog* successMessage = new SuccessDialog;
                successMessage->setWindowTitle("Congratulations!");
                successMessage->show();
                this->hide();
        }
    }else{
...
*/
void B3::labelRandMove(QLabel* label_ptr){
